...
interface EditableCaddieFieldProps {
  label: string;
  value: string | number;
...
export const EditableCaddieField: React.FC<EditableCaddieFieldProps> = ({
  label,
  value,
  onSave,
  type = "text",
  ...
  // select 타입일 때 value에 해당하는 label 찾기
      ...
      <div className="flex-1 flex items-center px-4 py-3 group">
        {isEditing ? (
          ...
          <div className="flex items-center justify-between w-full">
            <span className="text-sm text-black">{getDisplayValue()}</span>
            {!disabled && (
              <button
                onClick={handleEdit}
